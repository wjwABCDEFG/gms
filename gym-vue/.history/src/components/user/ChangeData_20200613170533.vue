<template>
  <div>
    ...
    <!-- 表格视图区 -->
    <div class="form">
        <el-form class="DataForm" ref="dataFormRef" :data="tableData" :rules="rules" label-position="right" label-width="80px" :model="PersonDataForm">
            <el-form-item label="用户id">
                <el-input v-model="PersonDataForm.user_id" disabled='false'></el-input>
            </el-form-item>
...
export default {
  ...
  methods: {
   async fillForm(){
    const {data:res} = await this.$http.post("getUserData",user.user_id);
